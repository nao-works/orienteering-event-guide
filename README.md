...
大会運営の正本生成では、イベント設定、入力検査、補正履歴、出力 manifest を残せる CLI / core workflow を優先します。Oスタ単体の出力を最終 Mulka 取込の根拠にしないでください。
...
| CLI / core workflow | 正本生成の基準 | JOY 正規化、イベント設定、検証、manifest、Mulka 出力 |

Oスタを運営利用する場合は、少なくとも重複カード番号、空カード、フリースタート、固定ゼッケン、複数レーン、複数日大会、入力ファイルの確定状態を別途確認してください。
